...
export interface StatSession {
  ...
  pause_count: number
  time_box: { date: string; start_min: number; end_min: number; task: StatTaskRef }
}
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        .from('focus_sessions')
        .select(
          'focused_seconds, pause_count, time_box:time_boxes!inner(date, start_min, end_min, task:tasks(id, name, color, category))'
        )
        .eq('state', 'done')
      ...
      supabase
        .from('time_boxes')
        .select('date, start_min, end_min, task:tasks(id, name, color, category)')
        .gte('date', startStr)
        .lte('date', endStr),
    ]).then(([s, b]) => {
      if (cancelled) return
